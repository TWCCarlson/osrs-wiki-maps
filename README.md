...
Python scripts are written with type hints introduced in 3.10, so ensure your Python version is 3.10+.
...
First, the `minimapIcons.json` file, which specifies the location definitions for icons, is loaded into the IconManager. This is done ahead of time because these values do not change but need to be referenced by all MapIDs. An icon definition looks like:
...
The instructions here specify a particular MapSquare or MapZone should be selected (sourced) and then moved (displayed) at a particular location. Each MapID is defined by lists of squares or zones to use in this manner. Only the squares or zones specified by the definition are drawn.
...
With the composite image produced, the pipeline is again restarted before zooming to reduce the number of pipelines run. For each zoom level specified in the configuration file, the image is rescaled by a factor of `2**<zoomLevel>`.

Each rescaled image is then sliced up into Leaflet-compatible tiles. The resulting directory doesn't have the correct structure, and so is coerced into the form `tiles/rendered/<MapID>/<zoomLevel>/<plane>_<x>_<y>.png`.
